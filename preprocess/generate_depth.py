...
if __name__ == '__main__':
    ...
    parser.add_argument('--images_dir', default="", help="Will be set to project_dir/images if not set")
    ...
    if args.images_dir == "":
        args.images_dir = os.path.join(args.project_dir, "images")
    ...
    images_dir = os.path.join(args.project_dir, "images")
    ...
    for cam_dir in cam_dirs:
        ...
        full_depth_path = os.path.join(args.project_dir, "depths", cam_dir)
    ...
    print(f"generating depth_params.json for chunks {os.listdir(args.project_dir)}.")
    try:
        subprocess.run([
            "python", "preprocess/make_depth_scale.py", "--base_dir", f"{args.project_dir}", "--depths_dir", f"{os.path.join(args.project_dir, 'depths')}"],
            check=True
        )
    except subprocess.CalledProcessError as e:
        print(f"Error executing run_monodepth: {e}")
